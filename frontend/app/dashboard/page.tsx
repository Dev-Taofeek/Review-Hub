...
import { staggerContainer, staggerItem, staggerFast, pageTransition, fadeInUp } from '@/lib/animations';
  ...
  return (
    <motion.div variants={reduced ? {} : pageTransition} initial="hidden" animate="visible"
      ...
      <div className="relative overflow-hidden border-b border-[var(--border)] bg-[var(--surface)]">
        <div className="absolute inset-0 signal-grid pointer-events-none" />
          ...
          <motion.div variants={reduced ? {} : staggerContainer} initial="hidden" animate="visible"
            ...
            <div className="flex items-center gap-5">
              ...
              <motion.div variants={reduced ? {} : staggerItem}>
                <p className="text-label-mono mb-1" style={{ color: 'var(--primary)' }}>{greeting}</p>
                <h1 className="font-black text-[var(--foreground)] tracking-[-0.03em] leading-tight"
            ...
            <motion.div variants={reduced ? {} : staggerItem} className="flex items-center gap-2 flex-wrap">
              ...
              <Link href="/profile">
                <button className="h-9 px-3.5 rounded-xl text-sm font-semibold transition-all"
                  style={{ background: 'var(--surface-soft)', border: '1px solid var(--border)', color: 'var(--foreground)' }}>
                  Edit Profile
                </button>
              </Link>
            </motion.div>
                ...
                <motion.div key={card.key} variants={reduced ? {} : staggerItem}
                  ...
                  <div className="relative">
                    ...
                    <AnimatedCounter value={raw} decimals={(card as any).dec ?? 0}
                      className="text-data text-3xl font-black text-[var(--foreground)] block leading-none mb-1.5" />
                    <p className="text-sm font-bold text-[var(--foreground)]">{card.label}</p>
                  ...
                  return (
                    <div key={item.key} className="rounded-xl p-3"
                      ...
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-data text-sm font-black text-[var(--foreground)]">{item.val}</span>
                      </div>
                  ...
                  {stats.recentReviews.map(review => {
                    ...
                    return (
                      <motion.div key={review.id} variants={reduced ? {} : staggerItem}
                        className="px-5 py-3.5 flex items-center gap-4 transition-colors cursor-pointer border-b border-[var(--border)] hover:bg-[var(--surface-soft)]"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-semibold text-[var(--foreground)] truncate mt-0.5">{truncate(review.title, 50)}</p>
            ...
            {[
              ...
              <motion.div key={action.href} variants={reduced ? {} : staggerItem}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-[var(--foreground)]">{action.label}</p>
