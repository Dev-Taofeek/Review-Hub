...
export function UserTable({ users, currentUserId, onUpdate }: UserTableProps) {
  ...
  return (
    <>
      <div className="overflow-x-auto rounded-xl border border-[var(--border)] dark:border-white/[0.07]">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[var(--muted)]">{user.username && `@${user.username}`}</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3"><RoleBadge role={user.role} /></td>
                <td className="px-4 py-3 text-sm text-[var(--muted)]">{user.review_count ?? 0}</td>
                <td className="px-4 py-3 text-xs text-[var(--muted)]">{user.created_at ? formatDate(user.created_at) : '—'}</td>
                <td className="px-4 py-3">
                  {user.is_banned
