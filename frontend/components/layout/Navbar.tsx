    ...
    <header className={cn(
      ...
      <div className="mx-auto flex h-[58px] max-w-[1600px] items-center justify-between px-4 xs:px-5 sm:px-8 lg:px-20">
            ...
            return (
              ...
              >
                {active && (
                  <motion.span
                    layoutId="nav-pill"
                    className="absolute inset-0 rounded-xl bg-slate-100 dark:bg-white/8 border border-slate-200/60 dark:border-white/10"
        ...
        <div className="flex items-center gap-2">
          {loading ? (
            <div className="h-8 w-8 animate-pulse rounded-full bg-slate-200 dark:bg-white/8" />
              ...
              <div className="relative" ref={dropRef}>
                ...
                >
                  <div className="relative">
                    ...
                    <div className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-white dark:border-[#06080F]"
                      style={{ background: 'var(--signal)' }} />
                  </div>
                  <span className="hidden sm:block text-sm font-semibold text-slate-700 dark:text-white max-w-[80px] truncate">
                    {user.username || user.full_name || 'Account'}
                  </span>
                  <motion.span animate={reduced ? {} : { rotate: profileOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
                    <ChevronDown className="hidden sm:block h-3.5 w-3.5 text-slate-400 dark:text-white/40" />
                  </motion.span>
                </motion.button>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => { closeAll(); router.push('/login'); }}
                className="h-8 px-3.5 rounded-xl text-sm font-semibold text-slate-600 hover:text-slate-900 dark:text-white/60 dark:hover:text-white transition-colors">
                Sign in
              </button>
              <motion.button
                whileHover={reduced ? {} : { scale: 1.02 }}
          ...
          <motion.button
            ...
            className={cn(
              ...
              mobileOpen
                ...
                : 'text-slate-600 dark:text-white hover:bg-slate-100 dark:hover:bg-white/[0.07]'
            )}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden bg-white dark:bg-[#0D1020] border-t border-slate-100 dark:border-white/[0.06]"
          >
            {/* User strip */}
            {isAuthenticated && user && (
              <div className="px-4 py-3 flex items-center gap-3 border-b border-slate-100 dark:border-white/[0.05]">
                <div className="relative">
                  <Avatar src={user.avatar_url} name={user.full_name || user.username} size="sm" />
                  <div className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-white dark:border-[#0D1020]"
                    style={{ background: 'var(--signal)' }} />
                </div>
            ...
            <div className="px-3 py-3 space-y-4">
              {/* Browse */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-white/30 px-3 mb-2">Discover</p>
                      ...
                      <Link key={link.href} href={link.href} onClick={closeAll}
                        className={cn(
                          ...
                          active
                            ...
                            : 'text-slate-600 dark:text-white/55 hover:bg-slate-50 dark:hover:bg-white/5'
                        )}>
                        <span className={cn(
              ...
              {isAuthenticated ? (
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-white/30 px-3 mb-2">Account</p>
                  <div className="space-y-0.5">
                      ...
                      return (
                        <Link key={link.href} href={link.href} onClick={closeAll}
                          ...
                          <span className={cn(
                            ...
                            active
                              ? 'bg-brand-100 dark:bg-[rgba(0,229,160,0.12)] text-brand-600 dark:text-[#00E5A0]'
                              : 'bg-slate-100 dark:bg-white/[0.06] text-slate-500 dark:text-white/35'
                          )}>
                            {link.icon}
                          </span>
                          {link.label}
                ...
                <div className="flex gap-2">
                  <Link href="/login" onClick={closeAll} className="flex-1">
                    <button className="w-full h-10 rounded-xl text-sm font-bold text-slate-700 dark:text-white bg-slate-100 dark:bg-white/[0.07] border border-slate-200 dark:border-white/10 transition-all">
                      Sign in
                    </button>
                  </Link>
              ...
              {/* Appearance */}
              <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-slate-200/60 dark:border-white/[0.07]">
                <span className="text-sm font-medium text-slate-600 dark:text-white/50">Appearance</span>
                <ThemeToggle />
              </div>
            </div>
          </motion.div>
...
function DropItem({ href, icon, label, onClick }: { href: string; icon: React.ReactNode; label: string; onClick?: () => void }) {
  return (
    <Link href={href} onClick={onClick}
      className="flex items-center gap-2.5 px-3 py-2 text-sm font-semibold rounded-xl transition-all text-slate-600 dark:text-white/65 hover:bg-slate-50 dark:hover:bg-white/[0.06] hover:text-slate-900 dark:hover:text-white group">
      <span className="flex h-7 w-7 items-center justify-center rounded-lg [&>svg]:h-3.5 [&>svg]:w-3.5 bg-slate-100 dark:bg-white/[0.07] text-slate-500 dark:text-white/45 group-hover:bg-brand-50 dark:group-hover:bg-[rgba(0,229,160,0.08)] group-hover:text-brand-600 dark:group-hover:text-[#00E5A0] transition-colors">
        {icon}
      </span>
      {label}
    </Link>
